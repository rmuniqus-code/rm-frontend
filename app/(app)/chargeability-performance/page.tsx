...
const LOC_COLORS: Record<string, string> = {
  // DB region names (from regions table via locations join)
  'India': '#44217A',
  'KSA':   '#BD1C7D',
  'Qatar': '#D4A017',
  'UAE':   '#0071e3',
  'USA':   '#10b981',
  // Legacy employee_region text values (fallback)
  'ME':    '#BD1C7D',
}
...
const PageHeader = styled.div`
  ...
  p { font-size: 13px; color: var(--color-text-secondary); margin-top: 3px; }
`
              ...
              {emp.currentProjects.map((p, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f5f5f5' }}>
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    <span style={{ color: allocationStatusColor(p.status), fontWeight: 600, fontSize: 12, textTransform: 'capitalize' }}>
                      {p.status || '—'}
                    </span>
                  </td>
...
/* ══════════════════════════════════════════════════════
   MAIN PAGE — all hooks at the top level, unconditionally
══════════════════════════════════════════════════════ */
export default function ChargeabilityPerformancePage() {
  ...
  /* ── Sort states — one per tab (all unconditional) ── */
  ...
  const subFuncOptions = useMemo(() => {
    const base = fDepts.length ? allEmployees.filter(e => fDepts.includes(e.department)) : allEmployees
  ...
  const locOptions = useMemo(() => {
    const base = fRegions.length ? allEmployees.filter(e => fRegions.includes(e.region)) : allEmployees
    return [...new Set(base.map(e => e.location).filter(Boolean))].sort()
  }, [allEmployees, fRegions])
  ...
  const filtered = useMemo((): CpEmployee[] => {
    ...
    return allEmployees.filter(e => {
      if (fDepts.length && !fDepts.includes(e.department)) return false
      if (fSubFuncs.length && !fSubFuncs.includes(e.subFunction)) return false
      if (fRegions.length && !fRegions.includes(e.region)) return false
      if (fLocs.length && !fLocs.includes(e.location)) return false
      if (fDesigs.length && !fDesigs.includes(e.designation)) return false
      if (fStatus.length) {
        const p = e.chargeabilityPct
        const ok = fStatus.some(s =>
  ...
  }, [allEmployees, fDepts, fSubFuncs, fRegions, fLocs, fDesigs, fStatus, search])
  ...
  /* ── ALL group data computed unconditionally at top level ── */
  ...
  const slRawStats = useMemo(() =>
    Object.keys(slGrp).map(d => ({ d, ...calcGroup(slGrp[d]) }))
  , [slGrp])

  const subRawStats = useMemo(() =>
    Object.keys(subGrp).map(k => ({ k, dept: subGrp[k][0]?.department ?? '', ...calcGroup(subGrp[k]) }))
  , [subGrp])

  const locRawStats = useMemo(() =>
    Object.keys(locGrp).map(k => ({ k, reg: locGrp[k][0]?.region ?? 'Other', ...calcGroup(locGrp[k]) }))
  , [locGrp])

  const desigRawStats = useMemo(() =>
    Object.keys(desigGrp).map((k, i) => ({ k, color: TREND_COLORS[i % TREND_COLORS.length], ...calcGroup(desigGrp[k]) }))
  , [desigGrp])

  const resSorted = useMemo(() =>
    [...filtered].sort((a, b) => a.chargeabilityPct - b.chargeabilityPct)
  , [filtered])

  /* ── Sorted versions (all at top level) ── */
  const slKey = useCallback((col: string, r: typeof slRawStats[0]) => {
    if (col === 'dept') return r.d
    if (col === 'resources') return r.count
    if (col === 'avail') return r.avail
    if (col === 'charged') return r.charged
    if (col === 'pct') return r.pct
    if (col === 'below') return r.below
    if (col === 'var') return r.pct - TARGET
    return r.pct
  }, [])
  const subKey = useCallback((col: string, r: typeof subRawStats[0]) => {
    if (col === 'sub') return r.k
    if (col === 'dept') return r.dept
    if (col === 'resources') return r.count
    if (col === 'avail') return r.avail
    if (col === 'charged') return r.charged
    if (col === 'pct') return r.pct
  ...
  const locKey = useCallback((col: string, r: typeof locRawStats[0]) => {
    if (col === 'loc') return r.k
    if (col === 'region') return r.reg
    if (col === 'resources') return r.count
    if (col === 'pct') return r.pct
    if (col === 'below') return r.below
    return r.pct
  ...
  const resKey = useCallback((col: string, r: CpEmployee) => {
    ...
    if (col === 'desig') return r.designation
    if (col === 'dept') return r.department
    if (col === 'sub') return r.subFunction
    if (col === 'loc') return r.location
    if (col === 'avail') return r.availableHours
    if (col === 'charged') return r.chargeableHours
    if (col === 'pct') return r.chargeabilityPct
    if (col === 'var') return r.chargeabilityPct - TARGET
    return r.chargeabilityPct
  }, [])
  ...
  /* ── Chart data (low-to-high order, independent of sort) ── */
  ...
  const desigChartData = useMemo(() => [...desigRawStats].sort((a, b) => a.pct - b.pct).map((s, i) => ({ name: s.k, pct: s.pct, below: s.below, color: s.color })), [desigRawStats])
  ...
  const hasFilters = !!(search || fDepts.length || fSubFuncs.length || fRegions.length || fLocs.length || fDesigs.length || fStatus.length)

  if (loading && !allEmployees.length) return <PageLoader message="Loading chargeability data…" />

  /* ════════════════════════════════════════════════════
     RENDER FUNCTIONS — NO hooks inside, only pure JSX
  ════════════════════════════════════════════════════ */
  ...
  const renderServiceLines = () => (
    <>
      ...
      <SectionSub>Chargeability vs 75% target · Deduped by Employee ID</SectionSub>
      <KpiRow>
      ...
      <ChartGrid>
        <ChartCard>
          <h3>Chargeability % by Service Line</h3><p className="sub">vs 75% target</p>
        ...
        <ChartCard>
          <h3>Resources Below 75% by Service Line</h3><p className="sub">Headcount at risk</p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={slChartData} margin={{ top: 16, right: 24, left: 8, bottom: 8 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
              <XAxis dataKey="name" angle={0} textAnchor="middle" interval={0} height={36} tick={{ fontSize: 11, fill: '#444' }} />
          ...
          <AccordionSection key={s.k}>
            ...
            <AccordionBody $open={isOpen}>
              {below.length === 0 ? (
                ...
                <TblWrap>
                  {/* table-layout:fixed + colgroup ensures identical column widths across every location accordion */}
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                    <colgroup>
                      <col style={{ width: '18%' }} />{/* Name */}
                      <col style={{ width: '14%' }} />{/* Designation */}
                      <col style={{ width: '11%' }} />{/* Dept */}
                      <col style={{ width: '18%' }} />{/* Sub-Function */}
                      <col style={{ width: '17%' }} />{/* Actual % bar */}
                      <col style={{ width: '8%' }}  />{/* Target % */}
                      <col style={{ width: '8%' }}  />{/* Variance */}
      ...
      <ChartGrid>
        <ChartCard>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={desigChartData} margin={{ top: 16, right: 24, left: 8, bottom: 8 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
              ...
              <Bar dataKey="pct" radius={[4, 4, 0, 0]} name="Chargeability %">
                {desigChartData.map((e, i) => <Cell key={i} fill={e.color} fillOpacity={0.78} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <ThresholdNote>— 75% target</ThresholdNote>
        ...
        <ChartCard>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={desigChartData} margin={{ top: 16, right: 24, left: 8, bottom: 8 }}>
              ...
              <Tooltip formatter={(v: any) => `${v} resources`} />
              <Bar dataKey="below" radius={[4, 4, 0, 0]} name="Below 75%">
                {desigChartData.map((e, i) => <Cell key={i} fill={`${e.color}88`} stroke={e.color} strokeWidth={1} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
  ...
  return (
    <PageWrap>
      ...
      <PageHeader>
        <div>
          <h1>Chargeability Performance Dashboard</h1>
          <p>vs 75% target · Service lines, sub-teams, locations, designations</p>
        </div>
      </PageHeader>
